<script setup>
</script>
<template>
  <div class="footer-section">
  <div class="footer-one">
    <ul>
      <li class="footer-one-head">Company</li>
      <li><RouterLink to="/about">About</RouterLink></li>
      <li><RouterLink to="/contact">Contact us</RouterLink></li>
      <li><RouterLink to="/faqs">FAQs</RouterLink></li>
    </ul>
    <ul>
      <li class="footer-one-head">Quick links</li>
      <li>find services</li>
      <li><RouterLink to="/pricing">pricing & plans</RouterLink></li>
      <li>List your business</li>
    </ul>
    <ul>
      <li class="footer-one-head">Resources</li>
      <li><RouterLink to="/blog">Blog</RouterLink></li>
      <li><RouterLink to="/affiliate">Affiliate Program</RouterLink></li>

    </ul>
    <ul>
      <li class="footer-one-head">More from Dutiful</li>
      <li>Dutiful jobs</li>
    </ul>
  </div>
  <div class="thereverse-case">
  <div class="footer-double">
    <div class="footer-left">
      <p>Follow us</p>
      <svg width="11" height="20" viewBox="0 0 11 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M9.12011 3.32003H11.0001V0.14003C10.0899 0.045377 9.17526 -0.00135428 8.26011 2.98641e-05C5.54011 2.98641e-05 3.68011 1.66003 3.68011 4.70003V7.32003H0.610107V10.88H3.68011V20H7.36011V10.88H10.4201L10.8801 7.32003H7.36011V5.05003C7.36011 4.00003 7.64011 3.32003 9.12011 3.32003Z" fill="#F5EEFF"/>
      </svg>
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M15 22.75H9C3.57 22.75 1.25 20.43 1.25 15V9C1.25 3.57 3.57 1.25 9 1.25H15C20.43 1.25 22.75 3.57 22.75 9V15C22.75 20.43 20.43 22.75 15 22.75ZM9 2.75C4.39 2.75 2.75 4.39 2.75 9V15C2.75 19.61 4.39 21.25 9 21.25H15C19.61 21.25 21.25 19.61 21.25 15V9C21.25 4.39 19.61 2.75 15 2.75H9Z" fill="#F5EEFF"/>
      <path d="M12 16.25C9.66 16.25 7.75 14.34 7.75 12C7.75 9.66 9.66 7.75 12 7.75C14.34 7.75 16.25 9.66 16.25 12C16.25 14.34 14.34 16.25 12 16.25ZM12 9.25C10.48 9.25 9.25 10.48 9.25 12C9.25 13.52 10.48 14.75 12 14.75C13.52 14.75 14.75 13.52 14.75 12C14.75 10.48 13.52 9.25 12 9.25Z" fill="#F5EEFF"/>
      <path d="M17 7.4999C16.87 7.4999 16.74 7.4699 16.62 7.4199C16.5 7.3699 16.39 7.2999 16.29 7.2099C16.2 7.1099 16.12 6.9999 16.07 6.8799C16.02 6.7599 16 6.6299 16 6.4999C16 6.3699 16.02 6.2399 16.07 6.1199C16.13 5.9899 16.2 5.8899 16.29 5.7899C16.34 5.7499 16.39 5.6999 16.44 5.6699C16.5 5.6299 16.56 5.5999 16.62 5.5799C16.68 5.5499 16.74 5.5299 16.81 5.5199C17.13 5.4499 17.47 5.5599 17.71 5.7899C17.8 5.8899 17.87 5.9899 17.92 6.1199C17.97 6.2399 18 6.3699 18 6.4999C18 6.6299 17.97 6.7599 17.92 6.8799C17.87 6.9999 17.8 7.1099 17.71 7.2099C17.61 7.2999 17.5 7.3699 17.38 7.4199C17.26 7.4699 17.13 7.4999 17 7.4999Z" fill="#F5EEFF"/>
      </svg>
      <svg width="20" height="18" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M20 2.80021C19.2483 3.1263 18.4534 3.34187 17.64 3.44021C18.4982 2.92753 19.1413 2.12099 19.45 1.17021C18.6436 1.65027 17.7608 1.98851 16.84 2.17021C16.2245 1.50278 15.405 1.05851 14.5098 0.907061C13.6147 0.755616 12.6945 0.905568 11.8938 1.3334C11.093 1.76123 10.4569 2.44274 10.0852 3.27105C9.71355 4.09935 9.62729 5.0276 9.84 5.91021C8.20943 5.82774 6.61444 5.40316 5.15865 4.66407C3.70287 3.92498 2.41885 2.8879 1.39 1.62021C1.02914 2.25038 0.839519 2.96403 0.84 3.69021C0.83872 4.36459 1.00422 5.02883 1.32176 5.62377C1.63929 6.21872 2.09902 6.72592 2.66 7.10021C2.00798 7.08247 1.36989 6.90751 0.8 6.59021V6.64021C0.804887 7.5851 1.13599 8.4993 1.73731 9.22818C2.33864 9.95705 3.17326 10.4559 4.1 10.6402C3.74326 10.7488 3.37287 10.806 3 10.8102C2.74189 10.8072 2.48442 10.7838 2.23 10.7402C2.49391 11.553 3.00462 12.2634 3.69107 12.7724C4.37753 13.2814 5.20558 13.5638 6.06 13.5802C4.6172 14.7155 2.83588 15.3351 1 15.3402C0.665735 15.3413 0.331736 15.3213 0 15.2802C1.87443 16.4905 4.05881 17.1329 6.29 17.1302C7.82969 17.1462 9.35714 16.8552 10.7831 16.2743C12.2091 15.6934 13.505 14.8341 14.5952 13.7467C15.6854 12.6593 16.548 11.3656 17.1326 9.94112C17.7172 8.51663 18.012 6.98994 18 5.45021C18 5.28021 18 5.10021 18 4.92021C18.7847 4.33502 19.4615 3.61763 20 2.80021Z" fill="#F5EEFF"/>
      </svg>
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M21.9656 6.54336C21.8795 6.19773 21.7108 5.87814 21.474 5.612C21.2373 5.34585 20.9395 5.14108 20.6063 5.01524C17.4 3.77774 12.2812 3.78711 12 3.78711C11.7188 3.78711 6.6 3.77774 3.39375 5.01524C3.06051 5.14108 2.76275 5.34585 2.52599 5.612C2.28923 5.87814 2.12054 6.19773 2.03438 6.54336C1.79063 7.47149 1.5 9.17774 1.5 11.9996C1.5 14.8215 1.79063 16.5277 2.03438 17.4559C2.12054 17.8015 2.28923 18.1211 2.52599 18.3872C2.76275 18.6534 3.06051 18.8581 3.39375 18.984C6.46875 20.1746 11.2875 20.2121 11.9344 20.2121H12.0656C12.7125 20.2121 17.5312 20.1746 20.6063 18.984C20.9395 18.8581 21.2373 18.6534 21.474 18.3872C21.7108 18.1211 21.8795 17.8015 21.9656 17.4559C22.2094 16.5277 22.5 14.8215 22.5 11.9996C22.5 9.17774 22.2094 7.47149 21.9656 6.54336ZM15.2062 12.309L10.7062 15.309C10.6466 15.3529 10.5741 15.376 10.5 15.3746C10.4379 15.3723 10.3771 15.3563 10.3219 15.3277C10.262 15.2966 10.2118 15.2494 10.1771 15.1915C10.1423 15.1336 10.1243 15.0672 10.125 14.9996V8.99961C10.1243 8.93207 10.1423 8.86566 10.1771 8.80774C10.2118 8.74982 10.262 8.70266 10.3219 8.67149C10.3817 8.63959 10.449 8.62449 10.5167 8.6278C10.5843 8.6311 10.6499 8.65268 10.7062 8.69024L15.2062 11.6902C15.2585 11.7232 15.3016 11.7688 15.3315 11.8229C15.3614 11.877 15.377 11.9378 15.377 11.9996C15.377 12.0614 15.3614 12.1222 15.3315 12.1763C15.3016 12.2304 15.2585 12.2761 15.2062 12.309Z" fill="#F5EEFF"/>
      </svg>
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M19.875 2.625H4.125C3.72718 2.625 3.34564 2.78304 3.06434 3.06434C2.78304 3.34564 2.625 3.72718 2.625 4.125V19.875C2.625 20.2728 2.78304 20.6544 3.06434 20.9357C3.34564 21.217 3.72718 21.375 4.125 21.375H19.875C20.2728 21.375 20.6544 21.217 20.9357 20.9357C21.217 20.6544 21.375 20.2728 21.375 19.875V4.125C21.375 3.72718 21.217 3.34564 20.9357 3.06434C20.6544 2.78304 20.2728 2.625 19.875 2.625ZM9 16.5C9 16.6989 8.92098 16.8897 8.78033 17.0303C8.63968 17.171 8.44891 17.25 8.25 17.25C8.05109 17.25 7.86032 17.171 7.71967 17.0303C7.57902 16.8897 7.5 16.6989 7.5 16.5V10.5C7.5 10.3011 7.57902 10.1103 7.71967 9.96967C7.86032 9.82902 8.05109 9.75 8.25 9.75C8.44891 9.75 8.63968 9.82902 8.78033 9.96967C8.92098 10.1103 9 10.3011 9 10.5V16.5ZM8.25 8.625C8.0275 8.625 7.80999 8.55902 7.62498 8.4354C7.43998 8.31179 7.29578 8.13609 7.21064 7.93052C7.12549 7.72495 7.10321 7.49875 7.14662 7.28052C7.19002 7.06229 7.29717 6.86184 7.4545 6.7045C7.61184 6.54717 7.81229 6.44002 8.03052 6.39662C8.24875 6.35321 8.47495 6.37549 8.68052 6.46064C8.88609 6.54578 9.06179 6.68998 9.1854 6.87498C9.30902 7.05999 9.375 7.2775 9.375 7.5C9.375 7.79837 9.25647 8.08452 9.0455 8.2955C8.83452 8.50647 8.54837 8.625 8.25 8.625ZM17.25 16.5C17.25 16.6989 17.171 16.8897 17.0303 17.0303C16.8897 17.171 16.6989 17.25 16.5 17.25C16.3011 17.25 16.1103 17.171 15.9697 17.0303C15.829 16.8897 15.75 16.6989 15.75 16.5V13.125C15.75 12.6277 15.5525 12.1508 15.2008 11.7992C14.8492 11.4475 14.3723 11.25 13.875 11.25C13.3777 11.25 12.9008 11.4475 12.5492 11.7992C12.1975 12.1508 12 12.6277 12 13.125V16.5C12 16.6989 11.921 16.8897 11.7803 17.0303C11.6397 17.171 11.4489 17.25 11.25 17.25C11.0511 17.25 10.8603 17.171 10.7197 17.0303C10.579 16.8897 10.5 16.6989 10.5 16.5V10.5C10.4995 10.3154 10.567 10.1371 10.6898 9.99926C10.8125 9.86139 10.9817 9.7736 11.1651 9.75271C11.3485 9.73182 11.5331 9.77928 11.6837 9.88603C11.8343 9.99277 11.9402 10.1513 11.9813 10.3312C12.4887 9.98723 13.0803 9.78795 13.6925 9.75481C14.3047 9.72166 14.9144 9.85591 15.4561 10.1431C15.9977 10.4303 16.4509 10.8597 16.767 11.385C17.083 11.9104 17.25 12.5119 17.25 13.125V16.5Z" fill="#F5EEFF"/>
      </svg>
    </div>
    <div class="footer-right">
      <p>Download the app</p>
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path fill-rule="evenodd" clip-rule="evenodd" d="M19.1001 19.16C19.6901 18.26 19.9101 17.8 20.3601 16.79C17.0401 15.53 16.5101 10.8 19.7901 8.98999C18.7901 7.72999 17.3801 7 16.0501 7C15.0901 7 14.4301 7.25001 13.8401 7.48001C13.3401 7.67001 12.8901 7.84 12.3301 7.84C11.7301 7.84 11.2001 7.65001 10.6401 7.45001C10.0301 7.23001 9.39006 7 8.59006 7C7.10006 7 5.51007 7.91 4.50007 9.47C3.08007 11.67 3.33007 15.79 5.62007 19.31C6.44007 20.57 7.54007 21.98 8.97007 22C9.57007 22.01 9.96007 21.83 10.3901 21.64C10.8801 21.42 11.4101 21.18 12.3401 21.18C13.2701 21.17 13.7901 21.42 14.2801 21.64C14.7001 21.83 15.0801 22.01 15.6701 22C17.1201 21.98 18.2801 20.42 19.1001 19.16Z" fill="#F5EEFF"/>
      <path fill-rule="evenodd" clip-rule="evenodd" d="M15.8399 2C15.9999 3.1 15.5499 4.19001 14.9599 4.95001C14.3299 5.77001 13.2299 6.41 12.1699 6.37C11.9799 5.31 12.4699 4.21999 13.0699 3.48999C13.7399 2.68999 14.8699 2.07 15.8399 2Z" fill="#F5EEFF"/>
      </svg>
      <svg width="20" height="22" viewBox="0 0 20 22" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M18.9622 9.70624L3.24033 0.696867C3.00863 0.564991 2.7463 0.49643 2.4797 0.498076C2.2131 0.499721 1.95164 0.571516 1.72158 0.706242C1.49617 0.836382 1.30903 1.02362 1.17901 1.2491C1.04899 1.47458 0.980681 1.73034 0.980958 1.99062V20.0094C0.983434 20.4056 1.14256 20.7847 1.4236 21.064C1.70464 21.3433 2.08475 21.5 2.48096 21.5C2.74658 21.4991 3.00771 21.4314 3.24033 21.3031L18.9622 12.2937C19.1951 12.1674 19.3896 11.9805 19.5251 11.7528C19.6607 11.5251 19.7322 11.265 19.7322 11C19.7322 10.735 19.6607 10.4749 19.5251 10.2472C19.3896 10.0195 19.1951 9.8326 18.9622 9.70624ZM11.4997 12.0594L13.2716 13.8312L5.00283 18.5562L11.4997 12.0594ZM5.00283 3.43437L13.2716 8.16874L11.4997 9.94062L5.00283 3.43437ZM14.6122 13.0531L12.5591 11L14.6122 8.94687L18.2122 11L14.6122 13.0531Z" fill="#F5EEFF"/>
      </svg>
    </div>
  </div>
  <div class="footer-end">
    <p class="last-thing">@ &nbsp; 2022 Dutiful</p>
    <p>Terms of service </p>
    <p>Privacy policy</p>
    <p>Disclaimer</p>
  </div>
  </div>  
</div>
</template>
<style>
.footer-section{
  background: #532F82;
  padding:3%;
  /* border:2px solid green; */
}
.footer-one{
  border: 1px solid #63428E;
  border-left: none;
  border-right: none;
  border-top: none;
  display:grid;
  /* padding:3%; */
  grid-template-columns: repeat(4,1fr);
  gap:1rem;

}
/* .footer-one ul{
  border:2px solid yellow;
 
} */
.footer-one ul li{
  padding:8px 0;
  font-family: 'Circular Std';
  font-style: normal;
  font-weight: 450;
  font-size: 16px;
  line-height: 20px;
  color: #B6B6E6;
  margin:2% 0;
  padding-left:20%;
  /* border:2px solid green; */
}
.footer-one ul li a{
  padding:0px 0;
  font-family: 'Circular Std';
  font-style: normal;
  font-weight: 450;
  font-size: 16px;
  line-height: 20px;
  color: #B6B6E6;

  /* border:2px solid green; */
}
.footer-one .footer-one-head{
  font-family: 'Circular Std';
  font-family: 'Circular Std';
font-style: normal;
font-weight: 500;
font-size: 22px;
line-height: 28px;
color: #FFFFFF;
margin-bottom: 10%;
}

.footer-double{
  /* border:2px solid yellow; */
  border: 1px solid #63428E;
  border-left: none;
  border-right: none;
  border-top: none;
  display:flex;
  padding:1% 0;
  justify-content: space-between;
}
.footer-left{
  /* border:2px solid green; */
  width:30%;
  font-family: 'Circular Std';
font-style: normal;
font-weight: 500;
font-size: 18px;
line-height: 23px;
padding:1% 0;
color: #FFFFFF;
  display:flex;
  justify-content: space-around;
}
.footer-right{
  width:20%;
  font-family: 'Circular Std';
  font-style: normal;
  font-weight: 500;
  font-size: 18px;
  line-height: 23px;
  padding:1% 0;
  color: #FFFFFF;
  
  display:flex;
  justify-content: space-around;
}
.footer-end{
  /* border:2px solid yellow; */
  margin-top:8px;
  width:45%;
  display:flex;
  font-family: 'Circular Std';
  font-style: normal;
  font-weight: 450;
  font-size: 16px;
  line-height: 20px;
  color: #B6B6E5;
  justify-content: space-around;
}

@media  screen and (max-width:920px){
  .footer-one{
    border: 1px solid #63428E;
    border-left: none;
    border-right: none;
    border-top: none;
    display:grid;
    /* padding:3%; */
    grid-template-columns: 1fr;
    gap:1rem;

  }
  .footer-one .footer-one-head{
    font-family: 'Circular Std';
    font-family: 'Circular Std';
    font-style: normal;
    font-weight: 500;
    font-size: 22px;
    line-height: 28px;
    color: #FFFFFF;
    margin-bottom: 5%;
  }
  .footer-one ul li{
    padding:8px 0;
    font-family: 'Circular Std';
    font-style: normal;
    font-weight: 450;
    font-size: 13px;
    line-height: 20px;
    color: #B6B6E6;
    margin:1% 0;
    padding-left:20%;
    /* border:2px solid green; */
  }
 
 
  .footer-right{
  width:25%;
  font-family: 'Circular Std';
  font-style: normal;
  font-weight: 500;
  font-size: 18px;
  line-height: 23px;
  padding:1% 0;
  color: #FFFFFF;
  
  display:flex;
  justify-content: space-around;
}
.footer-end{
  /* border:2px solid yellow; */
  margin-top:8px;
  width:80%;
  margin: auto;
  display:flex;
  font-family: 'Circular Std';
  font-style: normal;
  font-weight: 450;
  font-size: 16px;
  line-height: 20px;
  color: #B6B6E5;
  justify-content: start;
 /* gap:0.5rem; */
 margin-left:10%;
}
.footer-double{
  /* border:2px solid yellow; */
  border: 1px solid #63428E;
  border-left: none;
  border-right: none;
  border-top: none;
  display:grid;
  padding:1% 0;
  grid-template-columns: 1fr;
}
.footer-left{
  /* border:2px solid green; */
  width:70%;
  margin:auto;
  font-family: 'Circular Std';
font-style: normal;
font-weight: 500;
font-size: 18px;
line-height: 23px;
padding:3% 0;
color: #FFFFFF;
  display:flex;
  justify-content:start;
  gap:1.5rem;
  margin-left:auto;
}
.footer-right{
  width:85%;
  font-family: 'Circular Std';
  font-style: normal;
  font-weight: 500;
  font-size: 18px;
  line-height: 23px;
  padding:3% 0;
  color: #FFFFFF;
  /* border:2px solid green; */
  display:flex;
  justify-content: start;
  gap:1rem;
  margin-left:auto;
}
}
</style>